				...
				if ($points->getBalance() != 0)
				{
					...
					else
					{
						if ($newPrice < $_product->getPrice())
						{
							...
							$data = array_merge($data , array('action' => 'true','PI' => $_product->getId(), 'price'=> Mage::helper('core')->currency($_product->getCurrentPrice()), 'bidder'=> $customerSession->getCustomer()->getName(), 'bidderTable' => "<tr><td>".Mage::helper('core')->currency($_product->getCurrentPrice())."</td><td>".$customerSession->getCustomer()->getName()."</td></tr>"));
								
							echo json_encode($data);
						}
						elseif ($newPrice == $_product->getPrice())
						{
							$bidHistory->setCustomerId($customerSession->getCustomerId());
							$bidHistory->setProductId($data['productId']);
							$bidHistory->setPrice($_product->getCurrentPrice());
